...
// ---- API base (accept either env name), no trailing slash ----
...
async function fetchLookup(address: string) {
  ...
  return { address: flat.address, geographies: flat.geographies as Geo, reps: (flat.stateRepresentatives || []) as Rep[] };
}
async function fetchVotesForRep(r: Rep) {
  const qs = new URLSearchParams({ person_id: r.id || "", name: r.name || "", district: r.district || "" });
...
export default function RepsPage() {
  ...
  return (
    <main className="mx-auto max-w-3xl p-6 space-y-6">
      ...
      {/* Quick visibility: which API are we hitting */}
